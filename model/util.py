...
def sample_and_group(npoint, radius, neighbor, xyz, feature):
    ...
    center_idx = ids_shuffle[:, :S]


    center_xyz = index_points(xyz, center_idx)  #
    center_feature = index_points(feature, center_idx)  #


    idx = knn_point(k, xyz, center_xyz)
    ...
    grouped_xyz_norm = grouped_xyz - center_xyz.view(B, S, 1, C_xyz)



    grouped_feature_center = grouped_feature - center_feature.view(B, S, 1, D)


    res_points = torch.cat([
        grouped_feature_center,
        center_feature.view(B, S, 1, D).repeat(1, 1, k, 1)
    ], dim=-1)
